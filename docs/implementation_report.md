...
`configs/full_stationary.json` and `configs/full_nonstationary.json` define 20-seed CPU studies at 200k and 300k interactions. Both environment variable `RL_RUN_CONTEXT=remote` and flag `--allow-full-run` are mandatory. The current remote Linux deployment requires Python >= 3.10, creates `.venv`, installs `requirements.txt`, `requirements-dev.txt`, and the editable package, then runs tests and smoke before formal execution. It caps BLAS libraries at one thread per worker, accounts for cgroup v2 `cpu.max`, preserves failed-seed manifests, verifies completeness, regenerates aggregates/figures, and packages analysis artifacts with SHA-256. The exact supported procedure is in `docs/REMOTE_ONE_CLICK_GUIDE_ZH.md`; older single-batch guides remain as historical references.
...
Four remote-only profiles contain exactly 1000 fixed, 1500 tuning, 1000 tuned-evaluation, and 700 norm-scaled runs. The production launcher supports ordered `all` execution, filters, dry-run, resume, preserved failed attempts, and retry. Aggregation fails closed on incomplete/mixed/non-finite results and writes robust CSVs, figures, `analysis_manifest.json`, and a Chinese summary. Packaging produces verified full and analysis-core `.tar.gz` archives plus `.sha256`. Final pre-deployment verification reported 95 tests passed; core smoke, legacy cross smoke, extension smoke, alpha tuning/evaluation/norm smoke, launcher dry-run, resume/retry, packaging, and SHA checks passed. The informative design pilot produced all 20 per-run summaries but its historical top-level aggregation manifest failed with `EmptyDataError`; that failure and the mixed scientific result are retained. No formal full profile was run locally.
