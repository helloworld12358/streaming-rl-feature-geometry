...
The utilization-adapter extension adds one state-only hook to both existing streaming loops rather than duplicating either loop. Missing adapter configuration and explicit identity share one canonical path. Residual RFF and tile coding are fixed, deterministic, condition-independent transformations; all new formal profiles keep the same norm-scaled SARSA(lambda) rule and write only the `adapter_summary_v1` run summary schema. The unified planner reports 1390 logical cells, 560 externally validated Stage A identity cells, 120 structural no-op cells, 90 Stage A-to-B2 reuse cells, and 620 new runs. Formal stages remain guarded by `RL_RUN_CONTEXT=remote` plus `--allow-full-run`, require a real storage report, execute sequentially, and fail before new work if the Stage A baseline is absent or incompatible.
