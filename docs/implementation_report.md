...
The utilization-adapter extension adds one hook after complete controller-input assembly in both existing streaming loops rather than duplicating either loop. Missing adapter configuration and explicit identity share one exact no-op path. Residual RFF and tile coding are fixed, deterministic, condition-independent transformations over `[observation, condition state, bias]`; they have no learned parameters or update. Stage A and B2 preserve the fixed-alpha cross-full source, while Stage B1 preserves the fixed-alpha core T-maze v3 source. The lightweight `adapter_summary_v1` profile stores only run evidence and summaries. The planner reports 1,390 logical cells. With no compatible historical baseline, 100 B2 identity/RFF cells are strictly deduplicated against planned Stage A cells and 1,290 physical runs remain; with all 560 Stage A identity cells compatible, 730 new physical runs remain. Baseline absence or incompatibility is reported fieldwise and moves identity into pending instead of shrinking the matrix. Formal stages remain guarded by `RL_RUN_CONTEXT=remote` plus `--allow-full-run`, require a real storage report, and execute sequentially.
