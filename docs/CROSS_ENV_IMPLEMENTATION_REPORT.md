...
The follow-up adds a fifth independently registered informative hidden-velocity environment, exact per-step cost decomposition, robust seed summaries, config-fixed catastrophic failures, condition-specific alpha selection, causal norm-scaled alpha, and true-decision probes. Four guarded profiles contain 1000/1500/1000/700 runs and remain unexecuted locally. The local informative design pilot used seeds 200–204 and retained mixed predictive results; details are in `HIDDEN_VELOCITY_INFORMATIVE_DESIGN.md`. The production-root-cause revision adds causal shrinkage/smoothed whitening updates, time-scale-consistent Gaussian-moment output statistics, real per-transition validity tracking, compact partitioned storage, strict commit/config/schema resume, and pilot-calibrated disk preflight. Its final local verification is 112 tests, 70/70 production smoke, 13/13 original-horizon known-failure reruns, and a 50/50 storage pilot with a conservative 22.742 GiB peak projection. The current workflow is `PRODUCTION_ROOT_CAUSE_FIX_AND_CLOUD_RUN_ZH.md`.
